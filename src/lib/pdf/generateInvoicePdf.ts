...
/**
 * Generate invoice PDF bytes for a given invoice ID.
 * Uses service-role client — no auth cookies needed.
 * Returns { pdfBytes, filename } or null if invoice not found.
 */
export async function generateInvoicePdf(invoiceId: string): Promise<{ pdfBytes: Uint8Array; filename: string } | null> {
  ...
  const { data: inv } = await supabase
    ...
    .select(`
      *,
      service_orders(so_number, complaint, cause, correction, mileage_at_service,
  ...
  function drawText(text: string, x: number, yPos: number, options?: { font?: typeof font, size?: number, color?: typeof black }) {
    page.drawText(text || '', { x, y: yPos, size: options?.size || 10, font: options?.font || font, color: options?.color || black })
  }
  ...
  y -= 16
  if (shop?.address) { drawText(shop.address, leftMargin, y, { size: 9, color: gray }); y -= 13 }
  const shopCityLine = [shop?.city, shop?.state, shop?.zip].filter(Boolean).join(', ')
  if (shopCityLine) { drawText(shopCityLine, leftMargin, y, { size: 9, color: gray }); y -= 13 }
  if (shop?.phone) { drawText(shop.phone, leftMargin, y, { size: 9, color: gray }); y -= 13 }
  if (shop?.email) { drawText(shop.email, leftMargin, y, { size: 9, color: gray }); y -= 13 }
  ...
  if (so?.complaint) { drawText('Complaint:', leftMargin, y, { font: fontBold, size: 8, color: gray }); drawText(so.complaint.substring(0, 80), leftMargin + 55, y, { size: 8, color: black }); y -= 12 }
  if (so?.cause) { drawText('Cause:', leftMargin, y, { font: fontBold, size: 8, color: gray }); drawText(so.cause.substring(0, 80), leftMargin + 55, y, { size: 8, color: black }); y -= 12 }
  if (so?.correction) { drawText('Correction:', leftMargin, y, { font: fontBold, size: 8, color: gray }); drawText(so.correction.substring(0, 80), leftMargin + 55, y, { size: 8, color: black }); y -= 12 }
  if (so?.complaint || so?.cause || so?.correction) y -= 6

  const groups = [{ type: 'labor', label: 'LABOR' }, { type: 'part', label: 'PARTS' }, { type: 'other', label: 'OTHER' }]
  for (const group of groups) {
    const groupLines = lines.filter((l: any) => {
      if (l.parts_status === 'canceled') return false
      if (group.type === 'other') return !['labor', 'part'].includes(l.line_type)
      return l.line_type === group.type
    })
    if (!groupLines.length) continue
    drawText(group.label, colX.desc, y, { font: fontBold, size: 8, color: gray }); y -= 14
    let groupTotal = 0
    for (const l of groupLines) {
      if (y < 80) { drawText('... continued', colX.desc, y, { size: 8, color: gray }); y -= 14; break }
      ...
      drawText(desc, colX.desc, y, { size: 9 }); drawText(String(qty), colX.qty, y, { size: 9 }); drawText('$' + price.toFixed(2), colX.rate, y, { size: 9 }); drawText('$' + lineTotal.toFixed(2), colX.amount, y, { size: 9 }); y -= 14
    }
    if (groupLines.length > 1) { drawText(group.label + ' Subtotal:', colX.rate - 30, y, { font: fontBold, size: 8, color: gray }); drawText('$' + groupTotal.toFixed(2), colX.amount, y, { font: fontBold, size: 9 }); y -= 14 }
    y -= 4
  }
  ...
  for (const t of totalsData) {
    ...
    drawText('$' + t.value.toFixed(2), colX.amount, y, { font: t.bold ? fontBold : font, size: t.bold ? 11 : 9 })
    y -= t.bold ? 18 : 14
  }

  if (inv.notes) {
    y -= 10; drawText('Notes:', leftMargin, y, { font: fontBold, size: 9, color: gray }); y -= 14
    const noteWords = inv.notes.split(' '); let currentLine = ''
    for (const word of noteWords) {
      const test = currentLine ? currentLine + ' ' + word : word
      if (font.widthOfTextAtSize(test, 9) > 400) { drawText(currentLine, leftMargin, y, { size: 9, color: gray }); y -= 13; currentLine = word } else { currentLine = test }
    }
    if (currentLine) { drawText(currentLine, leftMargin, y, { size: 9, color: gray }); y -= 13 }
  }

  if (inv.due_date) { y -= 6; drawText('Payment due by ' + inv.due_date, leftMargin, y, { size: 9, color: gray }) }

  if ((inv.balance_due || 0) > 0 && shop?.payment_payee_name) {
    y -= 18; drawLine(leftMargin, y + 4, rightEdge, blue); y -= 4
    drawText('PAYMENT INSTRUCTIONS', leftMargin, y, { font: fontBold, size: 9, color: blue }); y -= 14
    ...
    for (const pl of payLines) { if (!pl) { y -= 6; continue }; if (y < 40) break; drawText(pl, leftMargin, y, { size: 8, color: gray }); y -= 11 }
    y -= 4; drawText(`Please include invoice number ${inv.invoice_number || ''} with your payment.`, leftMargin, y, { size: 8, color: gray })
  }
