...
export function generateInvoiceHTML(data: InvoicePDFData): string {
  ...
  const lineRows = (lines: typeof data.lines, color: string) => lines.map(l => `
    <tr>
      <td style="padding:6px 8px;font-size:11px;color:#1A1A24;border-bottom:1px solid #EDEDF0">${l.description}${l.part_number ? `<div style="font-size:9px;color:#888;font-family:monospace">${l.part_number}</div>` : ''}</td>
      <td style="padding:6px 8px;text-align:center;font-family:monospace;font-size:11px;color:#555;border-bottom:1px solid #EDEDF0">${l.quantity}</td>
      <td style="padding:6px 8px;text-align:right;font-family:monospace;font-size:11px;color:#555;border-bottom:1px solid #EDEDF0">$${l.unit_price.toFixed(2)}</td>
      <td style="padding:6px 8px;text-align:right;font-family:monospace;font-size:11px;font-weight:700;color:${color};border-bottom:1px solid #EDEDF0">$${l.total_price.toFixed(2)}</td>
    </tr>
  `).join('')
...
<style>
  * { margin:0; padding:0; box-sizing:border-box }
  body { font-family:'Helvetica Neue',Arial,sans-serif; background:#fff; color:#1A1A24; font-size:12px; }
  @media print {
...
<body style="max-width:800px;margin:0 auto;padding:32px">
  ...
  <!-- Header -->
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;padding-bottom:20px;border-bottom:2px solid #00E0B0">
    <div>
      <div style="font-size:24px;font-weight:800;letter-spacing:.5px;color:#1A1A24">${shopName.toUpperCase()}</div>
    ...
    <div style="text-align:right">
      <div style="font-size:28px;font-weight:800;color:${isPaid ? '#00E0B0' : '#00E0B0'}">${isPaid ? 'PAID' : 'INVOICE'}</div>
      <div style="font-family:monospace;font-size:14px;font-weight:700;color:#1A1A24;margin-top:4px">${data.invoice.invoice_number}</div>
  ...
  <div style="background:#EDEDF0;border-left:3px solid #00E0B0;padding:10px 14px;margin-bottom:20px;border-radius:0 6px 6px 0">
  ...
  <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
    ...
    <tbody>
      ${laborLines.length ? `<tr><td colspan="4" style="padding:5px 8px;font-size:9px;letter-spacing:1px;text-transform:uppercase;background:#EDEDF0;color:#00E0B0;font-weight:700">Labor</td></tr>${lineRows(laborLines,'#00E0B0')}` : ''}
      ${partLines.length  ? `<tr><td colspan="4" style="padding:5px 8px;font-size:9px;letter-spacing:1px;text-transform:uppercase;background:#EDEDF0;color:#00E0B0;font-weight:700">Parts</td></tr>${lineRows(partLines,'#00E0B0')}` : ''}
      ${otherLines.length ? `<tr><td colspan="4" style="padding:5px 8px;font-size:9px;letter-spacing:1px;text-transform:uppercase;background:#EDEDF0;color:#FFB84D;font-weight:700">Other</td></tr>${lineRows(otherLines,'#FFB84D')}` : ''}
    </tbody>
  </table>
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:24px">
    <div style="min-width:240px">
      ${[
        ...
        <div style="display:flex;justify-content:space-between;padding:5px 0;${r.bold ? 'border-top:2px solid #1A1A24;margin-top:4px' : 'border-bottom:1px solid #EDEDF0'}">
          <span style="font-size:${r.bold?'14':'11'}px;${r.bold?'font-weight:800':'color:#555'}">${r.label}</span>
          <span style="font-family:monospace;font-size:${r.bold?'18':'11'}px;font-weight:${r.bold?800:400};color:${r.bold?(isPaid?'#00E0B0':'#00E0B0'):'#333'}">${r.val}</span>
        </div>
      `).join('')}
    </div>
  </div>
  ...
  <div style="display:flex;align-items:center;gap:20px;background:#EDEDF0;border:2px solid #00E0B0;border-radius:10px;padding:16px;margin-bottom:24px">
    ${data.qrCodeUrl ? `<img src="${data.qrCodeUrl}" width="80" height="80" style="border-radius:6px"/>` : ''}
    <div>
      <div style="font-weight:700;font-size:13px;color:#00E0B0">Pay Online</div>
      <div style="font-size:11px;color:#555;margin-top:3px">Scan QR code or visit:</div>
      <div style="font-family:monospace;font-size:10px;color:#00E0B0;word-break:break-all">${data.paymentUrl}</div>
    </div>
  </div>` : ''}
  ...
  <div class="no-print" style="margin-top:24px;text-align:center">
    <button onclick="window.print()" style="padding:10px 24px;background:#00E0B0;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer">Print / Save as PDF</button>
  </div>
