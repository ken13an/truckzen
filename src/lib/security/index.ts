...
export async function validateInvoiceBeforeClose(
  ...
  if (!so?.assigned_tech || !so?.users?.full_name) {
  ...
  if (!invoice.customers?.email && !invoice.customers?.phone) {
...
Customer: ${invoice.customers?.company_name}
Technician: ${so?.users?.full_name || 'NOT ASSIGNED'}
Total: $${invoice.total}
...
export const rateLimits = {
  // Login: 5 attempts per minute per IP — after that, locked for 1 minute
  login: new Ratelimit({
    redis,
  ...
  // AI calls: 30 per minute per user — Claude API costs money
  ai: new Ratelimit({
    redis,
  ...
  // Telegram bot: 20 commands per minute per user
  telegram: new Ratelimit({
    redis,
  ...
  // Smart Drop: 10 imports per hour per user
