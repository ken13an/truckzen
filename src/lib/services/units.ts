...
export async function getUnits(shopId: string, customerId?: string): Promise<Unit[]> {
  ...
  let q = supabase.from('assets').select('*, customers(company_name)').eq('shop_id', shopId).order('unit_number')
...
export async function searchUnits(shopId: string, query: string, customerId?: string): Promise<Unit[]> {
  const supabase = createClient()
  let q = supabase.from('assets').select('id, unit_number, year, make, model, vin, unit_type').eq('shop_id', shopId).ilike('unit_number', `%${query}%`).limit(10)
  if (customerId) q = q.eq('customer_id', customerId)
  const { data } = await q
  return (data || []) as Unit[]
}
