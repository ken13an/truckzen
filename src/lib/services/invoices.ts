...
export async function getInvoices(shopId: string, status?: InvoiceStatus): Promise<Invoice[]> {
  ...
  let q = supabase.from('invoices').select('*, customers(company_name), service_orders(so_number, assets(unit_number))').eq('shop_id', shopId).is('deleted_at', null).order('created_at', { ascending: false })
