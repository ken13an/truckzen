...
const REDIRECT_URI  = `${process.env.NEXT_PUBLIC_APP_URL}/api/integrations/qbo/callback`
...
export async function pushInvoiceToQBO(invoiceId: string, shopId: string): Promise<{ success: boolean; qbo_id?: string; error?: string }> {
  ...
  if (qboId) {
    await supabase.from('invoices').update({ qbo_invoice_id: qboId, qbo_synced: true }).eq('id', invoiceId)
  }
