...
export function InstallPrompt() {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#F5F5F7' }}>Install TruckZen</div>
      ...
      {!isIOS && (
        <button onClick={handleInstall} style={{
          ...
          background: 'linear-gradient(135deg,#0A84FF,#0A84FF)', color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap',
        }}>Install</button>
      )}
      <button onClick={dismiss} style={{
        background: 'none', border: 'none', color: '#8E8E93', cursor: 'pointer', fontSize: 18, padding: 4, lineHeight: 1,
      }}>x</button>
    </div>
  )
}
