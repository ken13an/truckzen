...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#08080C' }}>
      <Sidebar/>
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        {/* Impersonation banner */}
        {isImpersonating && (
          <div style={{ background: 'rgba(245,158,11,.1)', borderBottom: '1px solid rgba(245,158,11,.2)', padding: '6px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 12, color: '#FFB84D', fontWeight: 600 }}>
            ...
            }} style={{ background: 'none', border: '1px solid rgba(245,158,11,.3)', borderRadius: 4, color: '#FFB84D', fontSize: 10, cursor: 'pointer', padding: '2px 8px', fontFamily: 'inherit' }}>
              Exit
            </button>
          </div>
        )}
