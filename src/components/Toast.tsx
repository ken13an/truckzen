...
const COLORS: Record<ToastType, { bg: string; border: string; icon: string; text: string }> = {
  success: { bg:'rgba(29,184,112,.1)',  border:'rgba(29,184,112,.3)',  icon:'✓', text:'#0A84FF' },
  error:   { bg:'rgba(217,79,79,.1)',   border:'rgba(217,79,79,.3)',   icon:'✗', text:'#FF453A' },
  warning: { bg:'rgba(212,136,42,.1)',  border:'rgba(212,136,42,.3)',  icon:'⚠', text:'#FFD60A' },
  info:    { bg:'rgba(0,224,176,.1)',  border:'rgba(0,224,176,.3)',  icon:'i', text:'#0A84FF' },
}
          ...
          return (
            <div key={t.id} style={{
              ...
              <button onClick={() => setToasts(prev => prev.filter(x => x.id !== t.id))}
                style={{ background: 'none', border: 'none', color: '#8E8E93', cursor: 'pointer', fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
            </div>
          )
        })}
      </div>
