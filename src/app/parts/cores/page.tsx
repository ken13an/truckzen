...
export default function CorePartsPage() {
  ...
  return (
    <div style={{ background: _t.bg, minHeight: '100vh', color: _t.text, fontFamily: FONT, padding: 24 }}>
      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: _t.text, marginBottom: 4 }}>Core Parts</div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: 'Shipped', value: String(shipped.length), color: _t.aiPurple },
          { label: 'Recovered', value: fmt(totalCredit), color: GREEN },
        ].map(s => (
          <div key={s.label} style={{ background: _t.bgCard, border: '1px solid rgba(255,255,255,.06)', borderRadius: 10, padding: '10px 16px', flex: 1, textAlign: 'center' }}>
            <div style={{ fontSize: 9, color: _t.textTertiary, textTransform: 'uppercase', letterSpacing: '.08em', fontFamily: MONO, marginBottom: 4 }}>{s.label}</div>
      ...
      {tabs[tab].data.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {tabs[tab].data.map((core: any) => {
            ...
            return (
              <div key={core.id} style={{ background: _t.bgCard, border: `1px solid ${isOverdue ? 'rgba(217,79,79,.3)' : 'rgba(255,255,255,.06)'}`, borderRadius: 12, padding: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: _t.text, marginBottom: 2 }}>{core.core_part_name}</div>
                    ...
                    {core.storage_location && <div style={{ fontSize: 10, color: _t.textTertiary, marginTop: 2 }}>Storage: {core.storage_location}</div>}
                    {core.tracking_number && <div style={{ fontSize: 10, color: _t.textTertiary, marginTop: 2 }}>Tracking: {core.tracking_number}</div>}
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    ...
                    {core.created_at && <div style={{ fontSize: 10, color: isOverdue ? RED : _t.textTertiary, marginTop: 4, fontWeight: isOverdue ? 700 : 400 }}>{daysSince(core.created_at)}d ago{isOverdue ? ' — OVERDUE' : ''}</div>}
