  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:'#0A84FF', marginBottom:4 }}>{part.part_number || 'No Part #'}</div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:'#F5F5F7' }}>{part.description}</div>
          <div style={{ fontSize:12, color:'#8E8E93', marginTop:4 }}>{part.category}</div>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <span style={{ padding:'4px 12px', borderRadius:100, fontFamily:'monospace', fontSize:9, background: isOut?'rgba(217,79,79,.1)':isLow?'rgba(212,136,42,.1)':'rgba(29,184,112,.1)', color: isOut?'#FF453A':isLow?'#FFD60A':'#0A84FF', border:`1px solid ${isOut?'#FF453A':isLow?'#FFD60A':'#0A84FF'}33` }}>
      ...
      {canEdit && edit && (
        <div style={S.card}>
          <div style={{ fontSize:13, fontWeight:700, color:'#F5F5F7', marginBottom:14 }}>Edit Part</div>
          <div style={S.row2}>
          ...
          <div style={S.row3}>
            ...
            <div><label style={S.label}>Margin</label><div style={{ padding:'9px 12px', background:'#0A0A0A', borderRadius:8, fontSize:13, fontWeight:700, color: margin>=40?'#0A84FF':margin>=25?'#FFD60A':'#FF453A', fontFamily:'monospace' }}>{margin}%</div></div>
          </div>
          <div style={S.row2}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))', gap:10, marginBottom:14 }}>
        {[
          { label:'On Hand',    val: part.on_hand,       color: isOut?'#FF453A':isLow?'#FFD60A':'#F5F5F7' },
          { label:'Reorder At', val: part.reorder_point, color:'#8E8E93' },
          { label:'Cost',       val: `$${(part.cost_price||0).toFixed(2)}`, color:'#8E8E93' },
          { label:'Sell Price', val: `$${(part.sell_price||0).toFixed(2)}`, color:'#F5F5F7' },
          { label:'Margin',     val: `${margin}%`, color: margin>=40?'#0A84FF':margin>=25?'#FFD60A':'#FF453A' },
          { label:'Core Charge',val: part.core_charge > 0 ? `$${part.core_charge}` : '—', color:'#FFD60A' },
        ].map(s => (
          <div key={s.label} style={{ background:'#2A2A2A', border:'1px solid rgba(255,255,255,.055)', borderRadius:10, padding:'10px 12px' }}>
            <div style={{ fontFamily:'monospace', fontSize:8, letterSpacing:'.1em', textTransform:'uppercase', color:'#8E8E93', marginBottom:5 }}>{s.label}</div>
