...
export default function DVIRPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    page:  { background:'#08080C', minHeight:'100vh', color:'#EDEDF0', fontFamily:"'Instrument Sans',sans-serif", padding:20, maxWidth:480, margin:'0 auto' },
    title: { fontFamily:"'Bebas Neue',sans-serif", fontSize:24, color:'#EDEDF0', marginBottom:4 },
    ...
    input: { width:'100%', padding:'10px 12px', background:'#1A1A24', border:'1px solid rgba(255,255,255,.08)', borderRadius:8, fontSize:13, color:'#EDEDF0', outline:'none', fontFamily:'inherit', minHeight:40, boxSizing:'border-box' as const, marginBottom:12 },
    btn:   { width:'100%', padding:14, background:'linear-gradient(135deg,#00E0B0,#00E0B0)', border:'none', borderRadius:10, color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit', marginTop:16 },
  }

  if (submitted) return (
    <div style={{ ...S.page, textAlign:'center', paddingTop:80 }}>
      <div style={{ fontSize:48 }}>{hasDefects ? '⚠️' : '✅'}</div>
      <div style={{ ...S.title, marginTop:16 }}>{hasDefects ? 'DVIR Submitted — Defects Reported' : 'DVIR Complete — No Defects'}</div>
      <div style={{ fontSize:13, color:'#9D9DA1', lineHeight:1.6, marginTop:8 }}>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ fontSize:11, color:'#9D9DA1', marginBottom:20 }}>Pre-trip or post-trip inspection report</div>
      ...
      {DVIR_ITEMS.map(cat => (
        <div key={cat.category} style={{ marginBottom:16 }}>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, letterSpacing:'.1em', textTransform:'uppercase', color:'#9D9DA1', marginBottom:8 }}>{cat.category}</div>
          {cat.items.map(item => (
            <div key={item} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 12px', background:'#1A1A24', border:'1px solid rgba(255,255,255,.06)', borderRadius:8, marginBottom:6 }}>
              <span style={{ fontSize:12, color:'#EDEDF0' }}>{item}</span>
                ...
                {(['ok','defect'] as const).map(v => (
                  <button key={v} onClick={() => setChecks(c => ({...c, [item]: checks[item]===v ? null : v}))}
                    style={{ padding:'4px 10px', borderRadius:6, fontSize:10, fontWeight:700, cursor:'pointer', fontFamily:'inherit', border:'none',
                      background: checks[item]===v ? (v==='ok'?'#00E0B0':'#FF5C5C') : '#1A1A24',
                      color: checks[item]===v ? '#fff' : '#9D9DA1' }}>
                    {v === 'ok' ? 'OK' : 'Defect'}
                  </button>
                ))}
              </div>
