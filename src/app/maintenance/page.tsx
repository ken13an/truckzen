    ...
    <div style={S.page}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:'#9D9DA1' }}>{pms.filter(p => p.next_due_date < today).length} overdue · {pms.length} total schedules</div>
        </div>
        <button onClick={() => window.location.href='/maintenance/new'} style={{ padding:'7px 14px', background:'linear-gradient(135deg,#00E0B0,#00E0B0)', border:'none', borderRadius:8, color:'#fff', fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>+ Add PM Schedule</button>
      </div>
      <div style={{ background:'#1A1A24', border:'1px solid rgba(255,255,255,.055)', borderRadius:12, overflow:'hidden' }}>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:520 }}>
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} style={{ ...S.td, textAlign:'center', color:'#9D9DA1' }}>Loading...</td></tr>
              : pms.map(pm => {
                ...
                const color   = isOver ? '#FF5C5C' : isSoon ? '#FFB84D' : '#00E0B0'
                return (
                  <tr key={pm.id} style={{ cursor:'pointer' }} onClick={() => window.location.href = '/maintenance/' + pm.id}>
                    ...
                    <td style={{ ...S.td, color:'#EDEDF0', fontWeight:600 }}>{pm.service_name}</td>
                    <td style={{ ...S.td, color:'#9D9DA1' }}>{pm.interval_miles ? `${pm.interval_miles.toLocaleString()} mi` : `${pm.interval_days}d`}</td>
                    <td style={{ ...S.td, fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color }}>{pm.next_due_date || '—'}</td>
                    <td style={{ ...S.td, fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:'#9D9DA1' }}>{pm.next_due_reading ? pm.next_due_reading.toLocaleString() : '—'}</td>
