  ...
  useEffect(() => {
    getCurrentUser(supabase).then(async (p: any) => {
      ...
      const [{ count: ov }, { count: ds }] = await Promise.all([
        supabase.from('maint_vehicle_renewals').select('*', { count: 'exact', head: true }).eq('shop_id', p.shop_id).eq('status', 'active').lt('expiry_date', today),
        supabase.from('maint_vehicle_renewals').select('*', { count: 'exact', head: true }).eq('shop_id', p.shop_id).eq('status', 'active').gte('expiry_date', today).lte('expiry_date', in30),
      ])
  ...
  return (
    <div style={{ background: 'var(--tz-bg)', minHeight: '100vh', color: 'var(--tz-text)', fontFamily: FONT, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: 'var(--tz-text)' }}>Vehicle Renewals</div>
        <a href="/maintenance/vehicle-renewals/new" style={{ padding: '8px 16px', background: 'linear-gradient(135deg,#1B6EE6,#1248B0)', border: 'none', borderRadius: 8, color: 'var(--tz-bgLight)', fontSize: 12, fontWeight: 700, textDecoration: 'none', fontFamily: FONT }}>+ New</a>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
        {[{ value: 'all', label: 'All' }, { value: 'active', label: 'Active' }, { value: 'renewed', label: 'Renewed' }].map(f => (
      ...
      <DataTable
        ...
        fetchData={async (page, limit, search) => {
          let url = `/api/maintenance/crud?table=maint_vehicle_renewals&shop_id=${shopId}&page=${page}&limit=${limit}&order_by=expiry_date&order_asc=true&search_cols=renewal_type,custom_name`
        ...
        emptyMessage="No vehicle renewals yet."
      />
    </div>
  )
}
