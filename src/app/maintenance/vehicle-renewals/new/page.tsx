...
const TYPES = ['registration', 'insurance', 'dot_annual', 'permit', 'ifta', 'ucr', 'other']

export default function NewVehicleRenewalPage() {
  ...
  const [form, setForm] = useState({ asset_id: '', renewal_type: 'registration', custom_name: '', expiry_date: '', reminder_days_before: '30', cost: '', notes: '' })
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.asset_id || !form.expiry_date) { setError('Truck and expiry date required'); return }
    setSaving(true); setError('')
    const res = await fetch('/api/maintenance/crud', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ table: 'maint_vehicle_renewals', shop_id: shopId, asset_id: form.asset_id, renewal_type: form.renewal_type, custom_name: form.custom_name || null, expiry_date: form.expiry_date, reminder_days_before: parseInt(form.reminder_days_before) || 30, cost: form.cost ? parseFloat(form.cost) : null, notes: form.notes || null, status: 'active' }) })
    if (!res.ok) { setError('Failed'); setSaving(false); return }
    router.push('/maintenance/vehicle-renewals')
  }
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    btn: { padding: '12px 24px', background: th.accent, border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, color: '#fff', cursor: 'pointer', fontFamily: 'inherit', width: '100%' },
  ...
  return (
    <div style={S.page}>
      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: th.text, marginBottom: 20 }}>New Vehicle Renewal</div>
      ...
      <form onSubmit={submit}>
        <div style={S.card}>
          ...
          <div style={{ marginBottom: 10 }}><label style={S.label}>Cost</label><input style={S.input} type="number" step="0.01" value={form.cost} onChange={set('cost')} /></div>
