...
export default function NewExpensePage() {
  ...
  useEffect(() => {
    getCurrentUser(supabase).then(async p => {
      ...
      const [a, d] = await Promise.all([
        supabase.from('assets').select('id, unit_number').eq('shop_id', p.shop_id).eq('status', 'active').order('unit_number'),
  ...
  return (
    <div style={S.page}>
      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: '#F0F4FF', marginBottom: 20 }}>New Expense</div>
