...
export default function TireTrackerPage() {
  ...
  function treadStatus(tire: any) {
    ...
    if (depth <= min) return { color: '#FF453A', label: 'ILLEGAL' }
    if (depth <= min + 2) return { color: '#FFD60A', label: 'LOW' }
    return { color: '#0A84FF', label: 'OK' }
  }
  ...
  for (const a of fleet) {
    for (const t of a.tires || []) {
      ...
      if (remaining <= 10000 && remaining > 0) alerts.push({ type: 'mileage', msg: `#${a.unit_number} ${POSITIONS.find(p => p.key === t.position)?.short || t.position}: ${remaining.toLocaleString()} mi remaining`, color: '#FFD60A', assetId: a.id })
      if (remaining <= 0) alerts.push({ type: 'expired', msg: `#${a.unit_number} ${POSITIONS.find(p => p.key === t.position)?.short || t.position}: PAST expected life`, color: '#FF453A', assetId: a.id })
      const ts = treadStatus(t)
      if (ts.label === 'ILLEGAL') alerts.push({ type: 'tread', msg: `#${a.unit_number} ${POSITIONS.find(p => p.key === t.position)?.short}: Tread ${t.current_tread}/32" — BELOW LEGAL MIN`, color: '#FF453A', assetId: a.id })
    }
  }
    ...
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#8E8E93' }}>{fleet.length} units · {fleet.reduce((s, a) => s + (a.tire_count || 0), 0)} active tires · {alerts.length} alerts</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: '#0A0A0A', borderRadius: 10, padding: 4 }}>
        {([['fleet', 'Fleet'], ['inventory', 'All Tires'], ['costs', 'Costs'], ['alerts', 'Alerts']] as const).map(([k, l]) => (
          <button key={k} onClick={() => { setTab(k); setSelectedAsset(null); if (k === 'costs') loadPrices(user.shop_id) }}
            style={{ flex: 1, padding: '10px 0', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: tab === k ? '#2A2A2A' : 'transparent', color: tab === k ? '#F5F5F7' : '#8E8E93' }}>
              ...
              <div key={a.id} onClick={() => { setSelectedAsset(a); loadAssetTires(user.shop_id, a.id) }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 9, color: '#8E8E93', textTransform: 'uppercase' }}>Worst tire</div>
                  </div>
                </div>
      ...
      {tab === 'fleet' && selectedAsset && (
        <>
          <button onClick={() => setSelectedAsset(null)} style={{ background: 'none', border: 'none', color: '#0A84FF', fontSize: 13, fontWeight: 600, cursor: 'pointer', padding: 0, marginBottom: 16 }}>← Back to fleet</button>
            ...
            {POSITIONS.map(pos => {
              ...
              if (!tire) return (
                <div key={pos.key} style={{ ...S.card, borderStyle: 'dashed', opacity: 0.5, textAlign: 'center', padding: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#8E8E93' }}>{pos.label}</div>
                  <div style={{ fontSize: 11, color: '#8E8E93', marginTop: 4 }}>Empty</div>
                </div>
              )
              ...
              return (
                <div key={pos.key} style={{ ...S.card, background: lifeBg(life.pct), borderColor: `${lifeColor(life.pct)}20` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#8E8E93', textTransform: 'uppercase' }}>{pos.label}</span>
                    <span style={{ fontSize: 18, fontWeight: 700, color: lifeColor(life.pct) }}>{life.pct}%</span>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#F5F5F7' }}>{tire.brand} {tire.model || ''}</div>
                  <div style={{ fontSize: 11, color: '#8E8E93', marginTop: 2 }}>{tire.size} {tire.is_recap ? '· RECAP' : ''}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, marginTop: 8, fontSize: 10, color: '#8E8E93' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                    ...
                    <button onClick={() => { setModalTire(tire); setModal('remove'); setF({}) }} style={{ ...S.smallBtn, color: '#FF453A', borderColor: '#FF453A' }}>Remove</button>
                  </div>
                  {tire.qr_token && <div style={{ fontSize: 9, color: '#8E8E93', marginTop: 6 }}>QR: {tire.qr_token}</div>}
                </div>
              )
            })}
          </div>
                  ...
                  {assetTires.filter(t => t.status !== 'active').map(t => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td style={{ ...S.td, color: t.status === 'failed' ? '#FF453A' : '#8E8E93', textTransform: 'uppercase', fontSize: 10, fontWeight: 600 }}>{t.status}</td>
            ...
            {(() => {
              ...
              return Object.entries(brands).sort((a, b) => {
                ...
                <div key={brand} style={S.card}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#F5F5F7' }}>{brand}</div>
                  <div style={{ fontSize: 22, fontWeight: 700, color: '#0A84FF', marginTop: 4 }}>
                    ${d.totalMiles > 0 ? (d.totalCost / d.totalMiles).toFixed(3) : '—'}
                    <span style={{ fontSize: 11, color: '#8E8E93', fontWeight: 400 }}>/mi</span>
                  </div>
                  <div style={{ fontSize: 11, color: '#8E8E93', marginTop: 4 }}>{d.count} tires · ${d.totalCost.toLocaleString()} total</div>
                </div>
              ))
            })()}
          </div>
      ...
      {tab === 'alerts' && (
        <>
          ...
          {alerts.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: '#8E8E93' }}>No tire alerts</div>}
          {alerts.map((a, i) => (
            <div key={i} onClick={() => { const asset = fleet.find(f => f.id === a.assetId); if (asset) { setTab('fleet'); setSelectedAsset(asset); loadAssetTires(user.shop_id, asset.id) } }}
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: a.color }}>{a.type === 'tread' ? 'ALERT' : a.type === 'expired' ? 'EXPIRED' : 'WARNING'}</div>
              <div>
                <div style={{ fontSize: 13, color: '#F5F5F7', fontWeight: 600 }}>{a.msg}</div>
                <div style={{ fontSize: 10, color: '#8E8E93', textTransform: 'uppercase', marginTop: 2 }}>{a.type}</div>
              </div>
            </div>
          ))}
        </>
          ...
          <div style={S.modalCard} onClick={e => e.stopPropagation()}>
            ...
            {modal === 'install' && <>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                <label style={{ fontSize: 12, color: '#8E8E93', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {modal === 'tread' && <>
              ...
              <div style={{ fontSize: 12, color: '#8E8E93', marginBottom: 12 }}>{modalTire?.brand} · {POSITIONS.find(p => p.key === modalTire?.position)?.label}</div>
            ...
            {modal === 'remove' && <>
              ...
              <div style={{ fontSize: 12, color: '#8E8E93', marginBottom: 12 }}>{modalTire?.brand} {modalTire?.size} · {POSITIONS.find(p => p.key === modalTire?.position)?.label}</div>
              ...
              <button onClick={removeTire} disabled={saving} style={{ ...S.bigBtn, marginTop: 16, background: '#FF453A', opacity: saving ? 0.5 : 1 }}>Remove Tire</button>
            </>}
...
const S: Record<string, React.CSSProperties> = {
  ...
  card: { background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: 12, padding: 16, marginBottom: 10 },
  sectionLabel: { fontSize: 11, fontWeight: 600, color: '#8E8E93', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 },
  fieldLabel: { fontSize: 11, fontWeight: 600, color: '#8E8E93', marginBottom: 4 },
  ...
  modalCard: { background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: 16, padding: 24, width: '100%', maxWidth: 480, maxHeight: '85vh', overflowY: 'auto' as const },
