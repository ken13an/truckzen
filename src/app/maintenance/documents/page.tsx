  ...
  return (
    <div style={{ background: 'var(--tz-bg)', minHeight: '100vh', color: 'var(--tz-text)', fontFamily: FONT, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: 'var(--tz-text)' }}>Documents</div>
      </div>

      <div style={{ background: 'var(--tz-bgCard)', border: `2px dashed ${'var(--tz-border)'}`, borderRadius: 12, padding: 30, textAlign: 'center', marginBottom: 16 }}>
        <Upload size={28} color={'var(--tz-textTertiary)'} style={{ marginBottom: 8 }} />
        <div style={{ color: 'var(--tz-textTertiary)', fontSize: 13 }}>Upload files to the maintenance document library</div>
        <div style={{ color: 'var(--tz-textTertiary)', fontSize: 11, marginTop: 4 }}>Drag & drop or click to upload. Uses maintenance-files storage.</div>
      </div>
      ...
      <DataTable
        columns={[
          { key: 'file_name', label: 'File Name', render: (r: any) => <span style={{ fontWeight: 600, color: 'var(--tz-text)' }}>{r.file_name}</span> },
