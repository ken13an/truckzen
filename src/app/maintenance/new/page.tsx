...
export default function NewPMPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    chip:  { padding:'5px 12px', borderRadius:100, fontSize:10, fontWeight:600, cursor:'pointer', border:'1px solid rgba(255,255,255,.08)', background:'#1C2130', color:'#7C8BA0', fontFamily:'inherit', transition:'all .13s', minHeight:30 },
  }
  ...
  <ChevronLeft size={16} strokeWidth={2} /> Maintenance
      ...
      <div style={{ fontSize:12, color:'#7C8BA0', marginBottom:20 }}>Set up a recurring maintenance schedule for a vehicle.</div>

      {error && <div style={S.error}>{error}</div>}

      {/* Quick templates */}
      <div style={S.card}>
        <div style={{ fontSize:12, fontWeight:700, color:'#F0F4FF', marginBottom:10 }}>Quick Templates</div>
      ...
      <form onSubmit={submit}>
        <div style={S.card}>
          ...
          <div style={{ fontSize:11, color:'#48536A', marginBottom:8 }}>Set one or both intervals — alert triggers when either is reached first.</div>
          <div style={S.row2}>
          ...
          <div style={{ fontSize:12, fontWeight:600, color:'#DDE3EE', margin:'14px 0 10px' }}>Next Service Due</div>
          <div style={S.row2}>
