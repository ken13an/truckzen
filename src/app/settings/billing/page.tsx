...
export default function BillingPage() {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ fontSize: 12, color: '#9D9DA1', marginBottom: 24 }}>{billing?.shop_name}</div>

      {/* Current Plan */}
      <div style={S.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#9D9DA1', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>Current Plan</div>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#EDEDF0' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 32, fontWeight: 700, color: '#00E0B0' }}>$0</div>
            <div style={{ fontSize: 11, color: '#9D9DA1' }}>/month</div>
          </div>
        </div>
        {plan === 'free' && (
          <div style={{ marginTop: 16, padding: '12px 16px', background: 'rgba(0,224,176,.06)', borderRadius: 8, border: '1px solid rgba(0,224,176,.15)' }}>
            ...
            <div style={{ fontSize: 11, color: '#9D9DA1', marginTop: 4 }}>You have full access to all TruckZen features during the beta period. Add a payment method to be ready when paid plans launch.</div>
          </div>
        )}
      </div>
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#9D9DA1', textTransform: 'uppercase', letterSpacing: '.06em' }}>Payment Methods</div>
          <button onClick={addPaymentMethod} style={S.btn}>+ Add Card</button>
        </div>

        {methods.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 24, color: '#9D9DA1', fontSize: 13 }}>
          ...
          methods.map((m: any) => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 28, background: '#1A1A24', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#9D9DA1', textTransform: 'uppercase' }}>
                  {m.brand}
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#EDEDF0', fontFamily: 'monospace' }}>**** {m.last4}</div>
                  <div style={{ fontSize: 11, color: '#9D9DA1' }}>Exp {m.exp_month}/{m.exp_year}</div>
                </div>
              </div>
              <button onClick={() => removePaymentMethod(m.id)}
                style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid rgba(239,68,68,.3)', background: 'none', color: '#FF5C5C', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                Remove
              </button>
            </div>
          ))
      ...
      {billing?.stripe_customer_id && (
        <div style={{ fontSize: 10, color: '#9D9DA1', marginTop: 8, fontFamily: 'monospace' }}>
...
const S: Record<string, React.CSSProperties> = {
  ...
  btn: { padding: '8px 16px', borderRadius: 8, border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg,#00E0B0,#00E0B0)', color: '#fff' },
}
