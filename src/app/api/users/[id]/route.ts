...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data: target } = await supabase.from('users').select('role, full_name').eq('id', id).eq('shop_id', user.shop_id).single()
  ...
  if (update.role && update.role !== target.role) {
    await log('user.role_changed', user.shop_id, user.id, { table:'users', recordId:id, oldData:{ role: target.role }, newData:{ role: update.role } })
  }
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  if (id === user.id) return NextResponse.json({ error: 'Cannot deactivate your own account' }, { status: 400 })

  await supabase.from('users').update({ active: false }).eq('id', id).eq('shop_id', user.shop_id)
  await log('user.deleted', user.shop_id, user.id, { table:'users', recordId:id })
