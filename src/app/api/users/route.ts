...
export async function POST(req: Request) {
  ...
  const { data: existingProfile } = await ctx.admin.from('users').select('id, email, shop_id').eq('email', normalizedEmail).single()
  if (existingProfile) {
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://truckzen.pro'
    try {
      const token = newInviteToken()
      const expiresAt = new Date(Date.now() + INVITE_EXPIRY_DAYS * 24 * 60 * 60 * 1000).toISOString()
      await ctx.admin.from('users').update({ invite_token: token, invite_expires_at: expiresAt, invite_accepted_at: null }).eq('id', existingProfile.id)
      const { data: shop } = await ctx.admin.from('shops').select('name, dba').eq('id', existingProfile.shop_id).single()
  ...
  const inviteExpiresAt = new Date(Date.now() + INVITE_EXPIRY_DAYS * 24 * 60 * 60 * 1000).toISOString()
