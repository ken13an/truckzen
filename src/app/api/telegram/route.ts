...
async function executeAndReply(parsed: any, user: any, chatId: number) {
  ...
  async function findSO() {
    ...
    if (so_number)    q = q.ilike('so_number', `%${so_number}%`)
      ...
      const { data: asset } = await supabase.from('assets').select('id').eq('shop_id', user.shop_id).ilike('unit_number', `%${truck_number}%`).single()
  ...
  switch (action) {
    case 'update_status': {
      ...
      await supabase.from('service_orders').update({ status: data?.new_status || 'in_progress' }).eq('id', so.id)
    ...
    case 'add_note': {
      ...
      const { data: cur } = await supabase.from('service_orders').select('internal_notes').eq('id', so.id).single()
      const updated = `${cur?.internal_notes || ''}\n[${ts} — ${user.full_name}]: ${note}`.trim()
      await supabase.from('service_orders').update({ internal_notes: updated }).eq('id', so.id)
    ...
    case 'flag_parts': {
      const so = await findSO()
      if (!so) { await sendTG(chatId, `❌ No open SO found`); return }
      await supabase.from('service_orders').update({ status: 'waiting_parts', priority: 'high' }).eq('id', so.id)
    ...
    case 'close_job': {
      const so = await findSO()
      if (!so) { await sendTG(chatId, `❌ No open SO found`); return }
      await supabase.from('service_orders').update({ status: 'good_to_go', completed_at: new Date().toISOString() }).eq('id', so.id)
