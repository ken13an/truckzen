...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data, error } = await supabase.from('so_lines').update(update).eq('id', params.id).select().single()
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  const { data: line } = await supabase.from('so_lines').select('so_id').eq('id', params.id).single()
  await supabase.from('so_lines').delete().eq('id', params.id)
