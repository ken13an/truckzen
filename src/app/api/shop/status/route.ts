...
export async function GET(req: Request) {
  ...
  if (token) {
    const { data: shop } = await supabase.from('shops').select('share_token').eq('id', shopId).single()
  ...
  const [
    ...
    supabase.from('service_orders')
      ...
      .limit(50),
    supabase.from('service_orders').select('id', { count: 'exact', head: true }).eq('shop_id', shopId).eq('status', 'waiting_parts'),
    supabase.from('service_orders').select('id', { count: 'exact', head: true }).eq('shop_id', shopId).eq('status', 'good_to_go'),
    supabase.from('invoices').select('id', { count: 'exact', head: true }).eq('shop_id', shopId).eq('status', 'sent').lt('due_date', today),
    supabase.from('pm_schedules').select('id', { count: 'exact', head: true }).eq('shop_id', shopId).eq('active', true).lt('next_due_date', today),
  ])
