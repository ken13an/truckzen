...
export async function POST(req: Request, { params }: P) {
  ...
  // actor role so 'override'-labeled lines fail-safe to blocked.
  const partsGate = await assertPartsRequirementResolved(s, wo.id, 'customer_portal')
  if (!partsGate.ok) {
    console.warn('[portal-new-items-approve] parts gate blocked', { woId: wo.id, failures: partsGate.failures })
