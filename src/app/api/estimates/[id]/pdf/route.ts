...
// Printable estimate route. Returns the SAME PDF as the email attachment —
// both call generateEstimatePdf so screen / printable / email-attachment
// always agree. Do NOT add inline PDF/HTML rendering here.
async function _GET(req: Request, { params }: Params) {
  ...
  const { data: est } = await s.from('estimates').select('id, approval_token').eq('id', id).single()
