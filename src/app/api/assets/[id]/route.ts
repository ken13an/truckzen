...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data: current } = await supabase.from('assets').select('*').eq('id', params.id).eq('shop_id', user.shop_id).single()
  ...
  const { data, error } = await supabase.from('assets').update(update).eq('id', params.id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  await log('so.updated' as any, user.shop_id, user.id, {
    table: 'assets', recordId: params.id,
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  await supabase.from('assets').update({ status: 'decommissioned' }).eq('id', params.id).eq('shop_id', user.shop_id)
