...
export async function GET(req: Request) {
  ...
  const { data: duePMs } = await supabase
    .from('pm_schedules')
  ...
  for (const [shopId, pms] of Object.entries(byShop)) {
    const { data: mgr } = await supabase.from('users').select('telegram_id').eq('shop_id', shopId).in('role', ['fleet_manager','maintenance_manager','owner','gm']).not('telegram_id','is',null).limit(1).single()
