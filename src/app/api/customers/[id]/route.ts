...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data, error } = await supabase.from('customers').update(update).eq('id', params.id).eq('shop_id', user.shop_id).select().single()
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  await supabase.from('customers').update({ active: false }).eq('id', params.id).eq('shop_id', user.shop_id)
