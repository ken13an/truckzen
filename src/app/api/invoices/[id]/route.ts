...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data: current } = await supabase.from('invoices').select('*').eq('id', params.id).eq('shop_id', user.shop_id).single()
  ...
  if (update.status && update.status !== current.status) {
    await log('invoice.sent' as any, user.shop_id, user.id, { table:'invoices', recordId:params.id, newData:{ status: update.status } })
  }
  return NextResponse.json(data)
}
