...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data, error } = await supabase.from('drivers').update(update).eq('id', params.id).eq('shop_id', user.shop_id).select().single()
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  await supabase.from('drivers').update({ status: 'inactive' }).eq('id', params.id).eq('shop_id', user.shop_id)
