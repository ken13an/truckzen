...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const { data: current } = await supabase.from('service_orders').select('so_number').eq('id', params.id).single()

  await supabase.from('service_orders').update({ status: 'void', updated_at: new Date().toISOString() }).eq('id', params.id).eq('shop_id', user.shop_id)

  await log('so.deleted', user.shop_id, user.id, {
    table: 'service_orders', recordId: params.id,
    oldData: current,
  })
