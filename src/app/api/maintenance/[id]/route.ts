...
export async function PATCH(req: Request, { params }: P) {
  ...
  const { data, error } = await supabase.from('pm_schedules').update(update).eq('id', params.id).eq('shop_id', user.shop_id).select().single()
...
export async function DELETE(_req: Request, { params }: P) {
  ...
  const user = await getCurrentUser(supabase)
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  await supabase.from('pm_schedules').update({ active: false }).eq('id', params.id).eq('shop_id', user.shop_id)
