...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#08080C] flex flex-col items-center justify-center p-5 relative overflow-hidden">
      ...
      <div className="w-full max-w-[400px] bg-[#111117] border border-[#1A1A24] rounded-xl p-8 relative z-10 shadow-2xl">
        {/* Logo */}
        <Logo size="md" className="mb-7" />

        {/* Heading */}
        <h1 className="text-xl font-bold text-[#EDEDF0] mb-1">Welcome back</h1>
        <p className="text-sm text-[#9D9DA1] mb-7">Sign in to your shop account</p>

        {/* Form */}
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          {/* Email */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="email" className="text-[10px] font-bold text-[#9D9DA1] uppercase tracking-widest font-mono">
              Email
            </label>
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-[#1A1A24] border border-[#1A1A24] rounded-md text-sm text-[#EDEDF0] placeholder:text-[#9D9DA1] outline-none focus:border-[#00E0B0] transition-colors duration-150 min-h-[44px]"
            />
          </div>

          {/* Password */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="password" className="text-[10px] font-bold text-[#9D9DA1] uppercase tracking-widest font-mono">
              Password
            </label>
            <div className="relative">
              <input
                ...
                className="w-full px-3.5 py-2.5 pr-11 bg-[#1A1A24] border border-[#1A1A24] rounded-md text-sm text-[#EDEDF0] placeholder:text-[#9D9DA1] outline-none focus:border-[#00E0B0] transition-colors duration-150 min-h-[44px]"
              />
              <button
                type="button" tabIndex={-1}
                onClick={() => setShowPass(p => !p)}
                aria-label={showPass ? 'Hide password' : 'Show password'}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9D9DA1] hover:text-[#9D9DA1] transition-colors duration-150 p-1"
              >
          ...
          {/* Error */}
          {error && (
            <div className="flex items-center gap-2 px-3 py-2.5 bg-[#FF5C5C]/10 border border-[#FF5C5C]/20 rounded-md text-xs text-[#FF5C5C] leading-snug">
          ...
          <button
            ...
            className={`w-full py-3 rounded-md text-sm font-bold mt-1 min-h-[48px] transition-all duration-150 ${
              loading
                ? 'bg-[#1A1A24] border border-[#1A1A24] text-[#9D9DA1] cursor-not-allowed'
                : 'bg-[#00E0B0] text-[#08080C] hover:bg-[#00E0B0] cursor-pointer shadow-[0_0_16px_rgba(0,224,176,0.25)]'
            }`}
          >
            {loading ? (
        ...
        {/* Footer */}
        <div className="text-center text-[11px] text-[#9D9DA1] mt-6 pt-5 border-t border-[#1A1A24]">
          No account? Contact your shop admin.
        </div>
      </div>

      {/* Bottom branding */}
      <div className="fixed bottom-4 text-[10px] text-[#9D9DA1] tracking-wider z-10 font-mono">
