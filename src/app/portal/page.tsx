...
export default function PortalPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    page:   { background:'#060708', minHeight:'100vh', color:'#DDE3EE', fontFamily:"'Instrument Sans',sans-serif", padding:'24px 16px', maxWidth:640, margin:'0 auto' },
    header: { display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 },
    title:  { fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:'#F0F4FF' },
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ fontSize:12, color:'#7C8BA0', marginBottom:20 }}>{customer?.customers?.company_name}</div>

      {loading ? <div style={{ color:'#7C8BA0', textAlign:'center', padding:40 }}>Loading your vehicles...</div>
      : orders.length === 0 ? <div style={{ color:'#7C8BA0', textAlign:'center', padding:40 }}>No service orders yet.</div>
      : orders.map(so => {
        ...
        return (
          <div key={so.id} style={S.card}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:10, borderTop:'1px solid rgba(255,255,255,.05)' }}>
              <div style={{ fontSize:11, color:'#48536A' }}>{new Date(so.created_at).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' })}</div>
