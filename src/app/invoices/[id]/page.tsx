...
export default function InvoiceDetailPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    th:     { fontFamily:"'IBM Plex Mono',monospace", fontSize:8, color:'#8E8E93', textTransform:'uppercase', letterSpacing:'.08em', padding:'7px 10px', textAlign:'left', background:'#0A0A0A' },
  ...
  const statusColor: Record<string, string> = { draft:'#8E8E93', sent:'#0A84FF', paid:'#0A84FF', voided:'#8E8E93' }
  const stColor = statusColor[invoice.status] || '#8E8E93'
      ...
      <>
        ...
        {items.map((l: any) => (
          <tr key={l.id}>
            ...
            <td style={{ ...S.td, fontFamily:'monospace', fontSize:10, color:'#8E8E93' }}>{l.part_number || '—'}</td>
            <td style={{ ...S.td, fontFamily:'monospace', textAlign:'center' }}>{l.quantity}</td>
            <td style={{ ...S.td, fontFamily:'monospace', color:'#8E8E93' }}>${(l.unit_price||0).toFixed(2)}</td>
  ...
  return (
    <div style={S.page}>
      <a href="/invoices" style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#8E8E93', textDecoration:'none', marginBottom:20 }}>← Invoices</a>

      {/* Header */}
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        <div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:12, color:'#0A84FF', fontWeight:700, marginBottom:4 }}>{invoice.invoice_number}</div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:'#F5F5F7' }}>{cust?.company_name}</div>
          <div style={{ fontSize:12, color:'#8E8E93', marginTop:4 }}>Unit #{asset?.unit_number} · {so?.so_number}</div>
        </div>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8 }}>
          ...
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:32, color: isPaid?'#0A84FF':'#0A84FF' }}>
            ${(invoice.total||0).toFixed(2)}
          </div>
          {invoice.balance_due > 0 && <div style={{ fontSize:12, color:'#8E8E93' }}>Balance due: ${invoice.balance_due.toFixed(2)}</div>}
        </div>
      </div>
      ...
      {checks && (checks.errors?.length > 0 || checks.warnings?.length > 0) && (
        <div style={{ background:'rgba(212,136,42,.06)', border:'1px solid rgba(212,136,42,.2)', borderRadius:10, padding:14, marginBottom:14 }}>
          {checks.errors?.map((e: string, i: number) => (
            <div key={i} style={{ display:'flex', gap:8, marginBottom:6, fontSize:12, color:'#FF453A' }}>
              <span>x</span> {e}
            </div>
          ))}
          {checks.warnings?.map((w: string, i: number) => (
            <div key={i} style={{ display:'flex', gap:8, marginBottom:6, fontSize:12, color:'#FFD60A' }}>
              <span>!</span> {w}
            </div>
          ))}
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:14, alignItems:'start' }}>
        <div>
          {/* Line items */}
          <div style={S.card}>
            <div style={{ fontSize:12, fontWeight:700, color:'#F5F5F7', marginBottom:12 }}>Line Items</div>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', minWidth:440 }}>
                <thead><tr>{['Description','Part #','Qty','Rate','Amount'].map(h => <th key={h} style={S.th as any}>{h}</th>)}</tr></thead>
                <tbody>
                  {lineSection('Labor', '#0A84FF', laborLines)}
                  {lineSection('Parts', '#0A84FF', partLines)}
                  {lineSection('Other', '#FFD60A', otherLines)}
                </tbody>
              </table>
            </div>
            {/* Totals */}
            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4, paddingTop:12, marginTop:8, borderTop:'1px solid rgba(255,255,255,.06)' }}>
              {[
                { label:'Subtotal', val: invoice.subtotal||0, color:'#F5F5F7' },
                { label:'Tax',      val: invoice.tax_amount||0, color:'#F5F5F7' },
                ...(invoice.amount_paid>0 ? [{ label:'Amount Paid', val: -(invoice.amount_paid||0), color:'#0A84FF' }] : []),
              ].map(r => (
                <div key={r.label} style={{ display:'flex', gap:40 }}>
                  <span style={{ fontSize:12, color:'#8E8E93' }}>{r.label}</span>
              ...
              <div style={{ display:'flex', gap:40, paddingTop:8, marginTop:4, borderTop:'1px solid rgba(255,255,255,.08)' }}>
                <span style={{ fontSize:15, fontWeight:700, color:'#F5F5F7' }}>Total</span>
                <span style={{ fontFamily:'monospace', fontSize:18, fontWeight:700, color: isPaid?'#0A84FF':'#0A84FF', minWidth:80, textAlign:'right' }}>${(invoice.total||0).toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          {so?.cause && (
            <div style={S.card}>
              ...
              <div style={{ fontSize:12, color:'#8E8E93', marginBottom:8 }}>Cause: <span style={{ color:'#F5F5F7' }}>{so.cause}</span></div>
              <div style={{ fontSize:12, color:'#8E8E93' }}>Correction: <span style={{ color:'#F5F5F7' }}>{so.correction}</span></div>
            </div>
          )}
        </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {!isPaid && invoice.status !== 'voided' && (
                <>
                  <button style={{ ...S.btn, background:'linear-gradient(135deg,#0A84FF,#0A84FF)', color:'#fff', width:'100%' }}
                    onClick={sendInvoice} disabled={sending}>
                    {sending ? 'Sending...' : invoice.status === 'draft' ? 'Send to Customer' : 'Resend Invoice'}
                  </button>
                  <button style={{ ...S.btn, background:'rgba(29,184,112,.1)', color:'#0A84FF', border:'1px solid rgba(29,184,112,.2)', width:'100%' }}
                    onClick={generateQR}>
                    Generate QR Payment
                  </button>
                  {qrUrl && (
                    <div style={{ textAlign:'center', padding:'12px 0' }}>
                      ...
                      <div style={{ fontSize:10, color:'#8E8E93', marginTop:6 }}>Customer scans to pay</div>
                    </div>
                  )}
                  <button style={{ ...S.btn, background:'rgba(212,136,42,.08)', color:'#FFD60A', border:'1px solid rgba(212,136,42,.2)', width:'100%' }}
              ...
              {isPaid && (
                <div style={{ textAlign:'center', padding:'12px 0' }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#0A84FF' }}>PAID</div>
                  <div style={{ fontSize:13, fontWeight:700, color:'#0A84FF', marginTop:6 }}>Paid</div>
                  {invoice.paid_at && <div style={{ fontSize:10, color:'#8E8E93', marginTop:4 }}>{new Date(invoice.paid_at).toLocaleDateString()}</div>}
                </div>
              )}
              <a href={`/orders/${so?.id}`} style={{ ...S.btn, background:'transparent', color:'#8E8E93', border:'1px solid rgba(255,255,255,.08)', textDecoration:'none', textAlign:'center', display:'block' }}>
                View Service Order →
              </a>
            </div>
          </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8, fontSize:12 }}>
              {[
                ...
                <div key={r.label} style={{ display:'flex', justifyContent:'space-between', paddingBottom:6, borderBottom:'1px solid rgba(255,255,255,.03)' }}>
                  ...
                  <span style={{ color:'#F5F5F7', textAlign:'right', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis' }}>{r.val}</span>
                </div>
              ))}
            </div>
          </div>
