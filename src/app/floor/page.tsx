...
const STATUS_CFG: Record<string, { label: string; color: string; dot: string }> = {
  not_started:           { label:'Not Started',          color:'rgba(72,83,106,.12)',    dot:'#9D9DA1' },
  not_approved:          { label:'Not Approved',          color:'rgba(212,136,42,.10)',   dot:'#FFB84D' },
  waiting_approval:      { label:'Waiting Approval',      color:'rgba(212,136,42,.10)',   dot:'#FFB84D' },
  in_progress:           { label:'In Progress',           color:'rgba(0,224,176,.10)',   dot:'#00E0B0' },
  waiting_parts:         { label:'Waiting Parts',         color:'rgba(232,105,42,.10)',   dot:'#FFB84D' },
  done:                  { label:'Done',                  color:'rgba(29,184,112,.10)',   dot:'#00E0B0' },
  ready_final_inspection:{ label:'Ready for Inspection',  color:'rgba(139,92,246,.10)',   dot:'#00E0B0' },
  good_to_go:            { label:'Good to Go',            color:'rgba(29,184,112,.14)',   dot:'#00E0B0' },
  failed_inspection:     { label:'Failed — Needs Work',   color:'rgba(217,79,79,.10)',    dot:'#FF5C5C' },
}
...
export default function FloorPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    title:   { fontFamily:"'Bebas Neue',sans-serif", fontSize:28, letterSpacing:'.02em', color:'#EDEDF0' },
    viewBtn: { padding:'6px 14px', borderRadius:7, fontSize:11, fontWeight:600, cursor:'pointer', border:'1px solid rgba(255,255,255,.08)', background:'#1A1A24', color:'#9D9DA1', fontFamily:'inherit' },
    viewOn:  { background:'rgba(0,224,176,.1)', color:'#00E0B0', border:'1px solid rgba(0,224,176,.3)' },
  }

  const pill = (status: string) => {
    const cfg = STATUS_CFG[status] || { label: status, color: 'rgba(72,83,106,.1)', dot: '#9D9DA1' }
    return (
    ...
    <div style={S.page}>
      <div style={S.header}>
        <div>
          ...
          <div style={{ fontSize:12, color:'#9D9DA1' }}>{jobs.length} active jobs · Live</div>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign:'center', padding:60, color:'#9D9DA1' }}>Loading floor...</div>
      ) : view === 'table' ? (
        // TABLE VIEW
              ...
              <thead>
                <tr>
                  {['Bay','Truck','Customer','Job','Tech','Status','Next Status'].map(h => (
                    <th key={h} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:8, color:'#9D9DA1', textTransform:'uppercase', letterSpacing:'.1em', padding:'7px 10px', textAlign:'left', background:'#08080C', whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={j.id} style={{ borderBottom:'1px solid rgba(255,255,255,.025)' }}>
                      ...
                      <td style={{ padding:'10px' }}>
                        <div style={{ fontWeight:700, color:'#EDEDF0' }}>#{asset?.unit_number}</div>
                        <div style={{ fontSize:9, color:'#9D9DA1' }}>{asset?.year} {asset?.make}</div>
                      </td>
                      <td style={{ padding:'10px', color:'#EDEDF0' }}>{cust?.company_name}</td>
                      <td style={{ padding:'10px', color:'#9D9DA1', fontSize:11, maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{j.complaint}</td>
                      <td style={{ padding:'10px', color:'#EDEDF0', fontSize:11 }}>{tech?.full_name || '—'}</td>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:10 }}>
          {STATUS_ORDER.filter(s => s !== 'void').map(status => {
            ...
            return (
              <div key={status} style={{ background:'#1A1A24', border:'1px solid rgba(255,255,255,.055)', borderRadius:11, overflow:'hidden' }}>
                <div style={{ padding:'10px 12px', borderBottom:'1px solid rgba(255,255,255,.055)', display:'flex', alignItems:'center', gap:7 }}>
                  ...
                  <span style={{ fontSize:11, fontWeight:700, color:'#EDEDF0' }}>{cfg.label}</span>
                  <span style={{ marginLeft:'auto', fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:'#9D9DA1' }}>{statusJobs.length}</span>
                </div>
                <div style={{ padding:8, display:'flex', flexDirection:'column', gap:6, minHeight:60 }}>
                  {statusJobs.length === 0 ? (
                    <div style={{ fontSize:10, color:'#9D9DA1', textAlign:'center', padding:10 }}>Empty</div>
                  ) : statusJobs.map(j => {
                    const asset = j.assets as any
                    const cust  = j.customers as any
                    return (
                      <div key={j.id} style={{ background:'#1A1A24', border:'1px solid rgba(255,255,255,.055)', borderRadius:8, padding:10, cursor:'pointer' }}
                        ...
                        <div style={{ fontSize:12, fontWeight:700, color:'#EDEDF0' }}>#{asset?.unit_number}</div>
                        <div style={{ fontSize:10, color:'#9D9DA1', marginTop:2 }}>{cust?.company_name}</div>
                        <div style={{ fontSize:10, color:'#9D9DA1', marginTop:4 }}>{j.bay || 'No bay'}</div>
                      </div>
                    )
                  })}
                </div>
