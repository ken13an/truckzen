...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setStats({
        open_jobs:       soData?.filter(s => !['good_to_go','void'].includes(s.status)).length ?? 0,
        in_progress:     soData?.filter(s => s.status === 'in_progress').length ?? 0,
        waiting_parts:   soData?.filter(s => s.status === 'waiting_parts').length ?? 0,
        good_to_go:      soData?.filter(s => s.status === 'good_to_go').length ?? 0,
        low_stock_parts: partsData?.filter(p => p.on_hand <= p.reorder_point).length ?? 0,
        overdue_invoices:invoiceData?.filter(i => i.due_date && i.due_date < today).length ?? 0,
        overdue_pm:      pmData?.filter(p => p.next_due_date && p.next_due_date < today).length ?? 0,
