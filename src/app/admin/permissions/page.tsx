...
export default function PermissionsPage() {
  ...
  sectionLabel: { fontSize: 11, fontWeight: 600, color: 'var(--tz-textSecondary)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 },
  table: { width: '100%', borderCollapse: 'collapse' as const, fontSize: 12 },
  th: { fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: 'var(--tz-textTertiary)', textTransform: 'uppercase' as const, letterSpacing: '.08em', padding: '8px 6px', textAlign: 'left' as const, background: 'var(--tz-bgInput)', whiteSpace: 'nowrap' as const },
  td: { padding: '6px 6px', borderBottom: `1px solid ${'var(--tz-border)'}`, fontSize: 11, color: 'var(--tz-textSecondary)' },
}
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--tz-textSecondary)' }}>{ALL_ROLES.length} roles · {MODULES.length} modules · {users.length} users</div>
      </div>

      {/* View tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: 'var(--tz-bgCard)', borderRadius: 10, padding: 4 }}>
        {([['roles', 'Role Permissions'], ['users', 'User Overrides'], ['audit', 'Audit Log']] as const).map(([k, l]) => (
          <button key={k} onClick={() => { setView(k); if (k === 'audit') loadAudit(user.shop_id) }}
            style={{ flex: 1, padding: '10px 0', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: view === k ? 'var(--tz-border)' : 'transparent', color: view === k ? 'var(--tz-text)' : 'var(--tz-textTertiary)' }}>{l}</button>
        ))}
      </div>
      ...
      {view === 'roles' && (
        <div style={{ overflowX: 'auto' }}>
          <table style={S.table}>
            <thead>
              <tr>
                <th style={{ ...S.th, position: 'sticky', left: 0, zIndex: 2, background: 'var(--tz-bgInput)', minWidth: 140 }}>Module</th>
                {ALL_ROLES.map(r => (
            ...
            <tbody>
              {MODULES.map(m => (
                <tr key={m.key}>
                  <td style={{ ...S.td, position: 'sticky', left: 0, background: 'var(--tz-bg)', zIndex: 1, fontWeight: 600, fontSize: 11, whiteSpace: 'nowrap' }}>
                    {m.icon} {m.label}
                  </td>
                  {ALL_ROLES.map(r => {
                      ...
                      <td key={r} style={{ ...S.td, textAlign: 'center', padding: '4px 2px' }}>
                        <button
                          ...
                          style={{
                            ...
                            background: allowed ? '#22C55E' : 'var(--tz-border)',
                            opacity: isUnlimited ? 0.4 : isDefault ? 0.7 : 1,
                            transition: 'background .15s',
                            position: 'relative',
                          }}>
                          <div style={{
                            width: 14, height: 14, borderRadius: '50%', background: 'var(--tz-bgLight)',
          ...
          <div style={{ fontSize: 10, color: 'var(--tz-textTertiary)', marginTop: 8 }}>
      ...
      {view === 'users' && (
        <>
            ...
            {users.map(u => (
              <button key={u.id} onClick={() => setSelectedUser(u)}
                style={{
                  ...
                  border: selectedUser?.id === u.id ? `1px solid ${'var(--tz-accent)'}` : `1px solid ${'var(--tz-border)'}`,
                  background: selectedUser?.id === u.id ? 'rgba(29,111,232,.1)' : 'var(--tz-bgCard)',
                  color: selectedUser?.id === u.id ? 'var(--tz-text)' : 'var(--tz-textSecondary)',
                }}>
                {u.full_name}
          ...
          {selectedUser && (
            <div style={S.card}>
              <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--tz-text)', marginBottom: 4 }}>{selectedUser.full_name}</div>
              <div style={{ fontSize: 12, color: 'var(--tz-textSecondary)', marginBottom: 16 }}>
                  ...
                  return (
                    <div key={m.key} onClick={() => toggleUserPerm(selectedUser.id, m.key)}
                      style={{
                        ...
                        border: hasOvr ? `1px solid ${'var(--tz-accent)'}` : `1px solid ${'var(--tz-border)'}`,
                        background: hasOvr ? 'rgba(29,111,232,.06)' : 'var(--tz-bgCard)',
                      }}>
                      <span style={{ fontSize: 12, color: effective ? 'var(--tz-text)' : 'var(--tz-textTertiary)' }}>{m.icon} {m.label}</span>
                      <div style={{
                        width: 28, height: 16, borderRadius: 8, background: effective ? '#22C55E' : 'var(--tz-border)',
                        position: 'relative',
                      }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: 'var(--tz-bgLight)', position: 'absolute', top: 3, left: effective ? 14 : 4, transition: 'left .15s' }} />
                      </div>
                    </div>
                  )
                })}
              </div>
              <div style={{ fontSize: 10, color: 'var(--tz-textTertiary)', marginTop: 8 }}>Click to toggle. Blue border = user-specific override. Click an override again to remove it and revert to role default.</div>
            </div>
          )}
        </>
      )}
      ...
      {view === 'audit' && (
        <>
          ...
          {auditLog.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: 'var(--tz-textTertiary)' }}>No permission changes recorded yet</div>}
          <table style={S.table}>
            <thead><tr>
