  ...
  useEffect(() => {
    async function load() {
      ...
      const [unitRes, custRes, woRes] = await Promise.all([
        ...
        supabase.from('service_orders').select('id, so_number, status, wo_status, complaint, grand_total, created_at, mileage_at_checkin, assigned_tech').eq('asset_id', unitId).order('created_at', { ascending: false }),
      ])
