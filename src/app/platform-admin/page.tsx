...
export default function PlatformOverview() {
  ...
  const statCards = [
    { label: 'Total Shops', value: stats.total_shops, icon: Store, color: t.accent },
    ...
    <div>
      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 12, padding: 20, marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <a href="/platform-admin/shops" style={{ fontSize: 11, color: t.accent, textDecoration: 'none' }}>View All</a>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['Shop Name', 'Owner', 'Plan', 'Status', 'WOs This Month', 'Joined'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '8px 12px', fontSize: 10, color: t.textTertiary, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: "'IBM Plex Mono', monospace", borderBottom: `1px solid ${t.border}`, fontWeight: 600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {(stats.recent_shops || []).map((shop: any) => (
              <tr key={shop.id}>
                <td style={{ padding: '10px 12px', fontSize: 13, color: t.text, borderBottom: `1px solid ${t.border}` }}>{shop.name}</td>
                <td style={{ padding: '10px 12px', fontSize: 12, color: t.textSecondary, borderBottom: `1px solid ${t.border}` }}>{shop.owner_name}</td>
                <td style={{ padding: '10px 12px', fontSize: 12, color: t.textSecondary, borderBottom: `1px solid ${t.border}` }}>{planLabel[shop.subscription_plan] || shop.subscription_plan}</td>
                ...
                <td style={{ padding: '10px 12px', fontSize: 12, color: t.textSecondary, borderBottom: `1px solid ${t.border}`, textAlign: 'center' }}>{shop.wo_this_month}</td>
                <td style={{ padding: '10px 12px', fontSize: 12, color: t.textTertiary, borderBottom: `1px solid ${t.border}` }}>{fmtDate(shop.created_at)}</td>
              </tr>
            ))}
      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 12, padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <a href="/platform-admin/activity" style={{ fontSize: 11, color: t.accent, textDecoration: 'none' }}>View All</a>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {(stats.recent_activity || []).map((a: any) => (
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderBottom: `1px solid ${t.border}` }}>
              <span style={{ fontSize: 9, fontWeight: 600, color: t.accent, background: 'rgba(29,111,232,.12)', padding: '2px 6px', borderRadius: 4, textTransform: 'uppercase', fontFamily: "'IBM Plex Mono', monospace", whiteSpace: 'nowrap' }}>
