...
export default function PlatformActivity() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#F0F4FF', margin: '0 0 20px' }}>Activity Log</h1>

      {/* Filters */}
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={actionFilter} onChange={e => { setActionFilter(e.target.value); if (user) fetchLogs(user.id, { action: e.target.value }) }} style={{ padding: '8px 12px', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, fontSize: 12, color: '#DDE3EE', outline: 'none', fontFamily: 'inherit' }}>
          {ACTION_TYPES.map(t => (
        ...
        <input
          ...
          style={{ padding: '8px 12px', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, fontSize: 12, color: '#DDE3EE', outline: 'none', fontFamily: 'inherit' }}
          placeholder="From"
        />
        <input
          type="date"
      ...
      <div style={{ background: '#0D0F12', border: '1px solid rgba(255,255,255,.08)', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {logs.map((log: any) => (
              <tr key={log.id}>
                <td style={{ padding: '10px 12px', fontSize: 11, color: '#48536A', borderBottom: '1px solid rgba(255,255,255,.04)', whiteSpace: 'nowrap', fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTime(log.created_at)}</td>
                ...
                <td style={{ padding: '10px 12px', fontSize: 12, color: '#DDE3EE', borderBottom: '1px solid rgba(255,255,255,.04)' }}>{log.description || '—'}</td>
                <td style={{ padding: '10px 12px', fontSize: 12, color: '#7C8BA0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>{log.shop_name || '—'}</td>
                <td style={{ padding: '10px 12px', fontSize: 12, color: '#7C8BA0', borderBottom: '1px solid rgba(255,255,255,.04)' }}>{log.performed_by_name || '—'}</td>
              </tr>
            ))}
            {logs.length === 0 && (
              <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: '#48536A', fontSize: 12 }}>No activity logged yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
