...
export default function PlatformImpersonate() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#F0F4FF', margin: '0 0 8px' }}>Impersonate / Enter Shop</h1>
      <p style={{ fontSize: 12, color: '#7C8BA0', margin: '0 0 24px' }}>Enter a shop as its owner to see exactly what they see and troubleshoot issues.</p>

      {/* Search */}
      <div style={{ position: 'relative', maxWidth: 480, marginBottom: 24 }}>
        <Search size={14} color="#48536A" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)' }} />
        <input
          value={search}
          onChange={e => handleSearch(e.target.value)}
          placeholder="Search shop by name or owner email..."
          autoFocus
          style={{ width: '100%', padding: '12px 14px 12px 38px', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 10, fontSize: 13, color: '#DDE3EE', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
        />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {shops.map(shop => (
          <div key={shop.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 18px', background: '#0D0F12', border: '1px solid rgba(255,255,255,.08)', borderRadius: 10 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#F0F4FF' }}>{shop.name}</div>
              <div style={{ fontSize: 11, color: '#7C8BA0', marginTop: 2 }}>{shop.owner_name} &middot; {shop.owner_email}</div>
            </div>
        ...
        {shops.length === 0 && (
          <div style={{ padding: 40, textAlign: 'center', color: '#48536A', fontSize: 12 }}>
