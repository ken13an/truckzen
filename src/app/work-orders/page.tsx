...
export default function WorkOrdersPage() {
  ...
  useEffect(() => {
    getCurrentUser(supabase).then(async (p) => {
      ...
      const url = showHistorical
        ? `/api/work-orders?shop_id=${p.shop_id}&limit=500`
        : `/api/work-orders?shop_id=${p.shop_id}&limit=500&historical=false`
  ...
  const historicalCount = orders.filter(o => o.is_historical).length
  ...
  const filtered = orders.filter(o => {
    ...
    if (dateRange !== 'all') {
      ...
      if (rangeStart && o.created_at) {
        if (new Date(o.created_at) < rangeStart) return false
      }
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F5F7', fontFamily: "'Instrument Sans', sans-serif", padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#7C8BA0', cursor: 'pointer', marginLeft: 8 }}>
          <input type="checkbox" checked={showHistorical} onChange={e => setShowHistorical(e.target.checked)} />
          Show Historical {showHistorical ? `(${historicalCount})` : ''}
        </label>
      </div>
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        {([['today', 'Today'], ['week', 'This Week'], ['month', 'This Month'], ['3months', 'Last 3 Months'], ['all', 'All Time']] as [DateRange, string][]).map(([v, l]) => (
          <button key={v} onClick={() => setDateRange(v)} style={{ padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: dateRange === v ? '1px solid #1D6FE8' : '1px solid #E5E7EB', background: dateRange === v ? '#EFF6FF' : '#fff', color: dateRange === v ? '#1D6FE8' : '#9CA3AF', fontFamily: 'inherit' }}>{l}</button>
        ))}
      </div>
              ...
              {paginated.map(o => {
                ...
                return (
                  <tr key={o.id} style={{ borderBottom: '1px solid #F3F4F6', cursor: 'pointer' }} onClick={() => window.location.href = `/work-orders/${o.id}`}
                    ...
                    <td style={{ padding: '12px 14px', fontFamily: 'monospace', fontSize: 12, fontWeight: 700, color: '#1D6FE8' }}>{o.so_number || '—'} <SourceBadge source={o.source} /> {o.is_historical && <span style={{ padding: '1px 6px', borderRadius: 4, background: 'rgba(124,139,160,0.1)', color: '#7C8BA0', fontSize: 9, fontWeight: 600, textTransform: 'uppercase' }}>Historical</span>}</td>
