...
export default function DriverDetailPage() {
  ...
  const expiryStatus = (date?: string) => {
    if (!date) return { color:'#8E8E93', label:'No date set' }
    if (date < today)     return { color:'#FF453A', label:'EXPIRED' }
    if (date <= in30days) return { color:'#FFD60A', label:'Expiring soon' }
    return { color:'#0A84FF', label:'Valid' }
  }

  const S: Record<string, React.CSSProperties> = {
    page:  { background:'#0A0A0A', minHeight:'100vh', color:'#F5F5F7', fontFamily:"'Instrument Sans',sans-serif", padding:24, maxWidth:720, margin:'0 auto' },
    card:  { background:'#2A2A2A', border:'1px solid rgba(255,255,255,.055)', borderRadius:12, padding:18, marginBottom:12 },
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        <div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:'#F5F5F7' }}>{driver.full_name}</div>
          <div style={{ fontSize:12, color:'#8E8E93', marginTop:4 }}>{(driver.customers as any)?.company_name}</div>
        </div>
        <span style={{ padding:'4px 12px', borderRadius:100, fontFamily:'monospace', fontSize:9, background: driver.status==='active'?'rgba(29,184,112,.1)':'rgba(72,83,106,.1)', color: driver.status==='active'?'#0A84FF':'#8E8E93', border:`1px solid currentColor` }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
        {[
          ...
          <div key={item.label} style={{ background:'#2A2A2A', border:`1px solid ${item.st.color}33`, borderRadius:12, padding:14 }}>
            <div style={{ fontFamily:'monospace', fontSize:8, letterSpacing:'.1em', textTransform:'uppercase', color:'#8E8E93', marginBottom:6 }}>{item.label}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:14, alignItems:'start' }}>
        <div>
          {/* Edit form */}
          <div style={S.card}>
            <div style={{ fontSize:12, fontWeight:700, color:'#F5F5F7', marginBottom:12 }}>Driver Details</div>
            <div style={S.row2}>
            ...
            <button style={{ ...S.btn, background:'linear-gradient(135deg,#0A84FF,#0A84FF)', color:'#fff' }} onClick={save} disabled={saving}>{saving?'Saving...':'Save Changes'}</button>
          </div>
        </div>

        {/* DVIR history sidebar */}
        <div style={S.card}>
          <div style={{ fontSize:12, fontWeight:700, color:'#F5F5F7', marginBottom:12 }}>Recent DVIRs</div>
          {dvirs.length === 0 ? <div style={{ color:'#8E8E93', fontSize:12, textAlign:'center', padding:16 }}>No DVIRs on file</div>
          : dvirs.map((d:any) => (
            <div key={d.id} style={{ padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,.04)' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <div style={{ fontSize:11, color:'#F5F5F7' }}>{d.submitted_at?.split('T')[0]}</div>
                <span style={{ fontSize:9, fontWeight:700, fontFamily:'monospace', color: d.defects_found?'#FF453A':'#0A84FF' }}>
                  {d.defects_found ? 'DEFECTS' : 'CLEAN'}
                </span>
              </div>
              <div style={{ fontSize:10, color:'#8E8E93', marginTop:2 }}>{d.trip_type === 'pre' ? 'Pre-Trip' : 'Post-Trip'}</div>
            </div>
          ))}
        </div>
      </div>
