...
export default function ServiceRequestsPage() {
  ...
  if (loading) return <div style={{ background: 'var(--tz-bg)', minHeight: '100vh', color: 'var(--tz-textSecondary)', fontFamily: "'Instrument Sans',sans-serif", padding: 40, textAlign: 'center' }}>Loading...</div>

  return (
    <div style={{ background: 'var(--tz-bg)', minHeight: '100vh', color: 'var(--tz-text)', fontFamily: "'Instrument Sans',sans-serif", padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: 'var(--tz-text)' }}>Service Requests</div>
          <div style={{ fontSize: 12, color: 'var(--tz-textSecondary)' }}>Intake requests from Fleet, Maintenance, and Kiosk</div>
        </div>
        <a href="/service-requests/new" style={{ padding: '8px 16px', background: 'var(--tz-accent)', border: 'none', borderRadius: 8, color: 'var(--tz-bgLight)', fontSize: 12, fontWeight: 700, textDecoration: 'none', fontFamily: 'inherit' }}>+ New Request</a>
      </div>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${'var(--tz-border)'}`, marginBottom: 20 }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => { setTab(t.key); setPage(1) }} style={{
            ...
            color: tab === t.key ? 'var(--tz-accentLight)' : 'var(--tz-textSecondary)', background: 'none', border: 'none',
            borderBottom: tab === t.key ? `2px solid ${'var(--tz-accentLight)'}` : '2px solid transparent',
      ...
      {/* Request cards */}
      {filtered.length === 0 ? (
        <div style={{ background: 'var(--tz-bgCard)', border: `1px solid ${'var(--tz-border)'}`, borderRadius: 12, padding: 40, textAlign: 'center', color: 'var(--tz-textSecondary)', fontSize: 13 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {paginated.map(r => {
            ...
            return (
              <div key={r.id} style={{ background: 'var(--tz-bgCard)', border: `1px solid ${'var(--tz-border)'}`, borderRadius: 12, padding: 16 }}>
                {/* Top row: badges + time */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--tz-textTertiary)' }}>{timeAgo(r.created_at)}</span>
                </div>

                {/* Truck + Customer */}
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--tz-text)' }}>
                    {r.unit_number ? `#${r.unit_number}` : '—'}{r.unit_number && ' — '}{r.company_name || 'Walk-in'}
                  </div>
                  {r.contact_name && <div style={{ fontSize: 11, color: 'var(--tz-textSecondary)', marginTop: 2 }}>{r.contact_name}{r.phone ? ` · ${r.phone}` : ''}</div>}
                </div>

                {/* Concern */}
                <div style={{ fontSize: 13, color: 'var(--tz-text)', marginBottom: 10, lineHeight: 1.5 }}>{r.description || '—'}</div>

                {/* Details row */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginBottom: 12, fontSize: 11 }}>
                  <div><span style={{ color: 'var(--tz-textTertiary)' }}>Arriving:</span> <span style={{ color: 'var(--tz-text)' }}>{fmtDate(r.expected_arrival) || fmtDate(r.scheduled_date) || '—'}</span></div>
                  <div><span style={{ color: 'var(--tz-textTertiary)' }}>Need by:</span> <span style={{ color: 'var(--tz-text)' }}>{fmtDate(r.promised_date) || '—'}</span></div>
                  <div><span style={{ color: 'var(--tz-textTertiary)' }}>Parking:</span> <span style={{ color: 'var(--tz-text)' }}>{r.parking_location || '—'}</span></div>
                  <div><span style={{ color: 'var(--tz-textTertiary)' }}>Keys:</span> <span style={{ color: 'var(--tz-text)' }}>{r.key_location || '—'}</span></div>
                </div>

                {/* Actions */}
                {r.status === 'new' || r.status === 'scheduled' ? (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => convertToWO(r)} style={{ padding: '6px 14px', background: 'rgba(29,111,232,.12)', color: 'var(--tz-accentLight)', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Convert to WO</button>
                    <a href={`/service-requests/new?edit=${r.id}`} style={{ padding: '6px 14px', background: 'var(--tz-border)', color: 'var(--tz-textSecondary)', borderRadius: 6, fontSize: 11, textDecoration: 'none', display: 'flex', alignItems: 'center' }}>Edit</a>
                  ...
                  <a href={getWorkorderRoute(r.converted_so_id, undefined, 'service-request')} style={{ fontSize: 11, color: 'var(--tz-accentLight)', textDecoration: 'none' }}>View Work Order →</a>
                ) : r.status === 'rejected' ? (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }} onClick={() => setRejectId(null)}>
          <div style={{ background: '#12131a', border: `1px solid ${'var(--tz-border)'}`, borderRadius: 16, padding: 28, width: 400 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button onClick={rejectRequest} disabled={!rejectReason.trim()} style={{ flex: 1, padding: 10, background: rejectReason.trim() ? '#D94F4F' : 'var(--tz-border)', color: rejectReason.trim() ? 'var(--tz-bgLight)' : 'var(--tz-textTertiary)', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: rejectReason.trim() ? 'pointer' : 'default', fontFamily: 'inherit' }}>Reject</button>
              <button onClick={() => setRejectId(null)} style={{ padding: '10px 20px', background: 'var(--tz-border)', color: 'var(--tz-textSecondary)', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
