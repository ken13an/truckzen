    ...
    <div style={{ minHeight: '100vh', background: '#F4F5F7', fontFamily: FONT, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#6B7280' }}>{data?.period?.from} to {data?.period?.to}</div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => { setPeriod(p.key); setSelectedMech(null) }} style={{
              ...
              background: period === p.key ? `${BLUE}10` : '#fff',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={c.label} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: '16px 20px' }}>
            <div style={{ fontSize: 10, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '.06em', fontFamily: MONO, marginBottom: 6 }}>{c.label}</div>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid #E5E7EB' }}>
              {['Name', 'Role', 'Jobs', 'Hours', 'Avg Time/Job', 'Productivity'].map(h => (
                <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '.04em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {mechanics.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: '#9CA3AF', fontSize: 13 }}>No mechanic data for this period</td></tr>
            ) : mechanics.map((m: any) => (
              <tr key={m.id} style={{ borderBottom: '1px solid #F3F4F6', cursor: 'pointer' }}
                ...
                <td style={{ padding: '12px 14px', fontSize: 11, color: '#6B7280' }}>{m.role?.replace(/_/g, ' ')}</td>
                ...
                <td style={{ padding: '12px 14px', fontFamily: MONO, fontSize: 12, color: '#6B7280' }}>{m.avg_time_per_job_min}m</td>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #E5E7EB' }}>
                {['WO #', 'Description', 'Completed', 'Total'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#6B7280', textTransform: 'uppercase' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {selectedMech.jobs.map((j: any) => (
                <tr key={j.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                  ...
                  <td style={{ padding: '10px 12px', fontSize: 12, color: '#6B7280' }}>{j.completed_at ? new Date(j.completed_at).toLocaleDateString() : '--'}</td>
