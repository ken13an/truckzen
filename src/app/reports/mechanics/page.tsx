...
export default function MechanicReportsPage() {
  ...
  if (loading && !data) return <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: t.textSecondary }}>Loading...</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: FONT, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ fontSize: 24, fontWeight: 800, color: t.text }}>Mechanic Reports</div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => { setPeriod(p.key); setSelectedMech(null) }} style={{
              ...
              border: period === p.key ? `1px solid ${BLUE}` : `1px solid ${t.border}`,
          ...
          <button onClick={exportCSV} style={{
            padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT,
            border: `1px solid ${t.border}`, background: t.bgLight, color: t.textSecondary,
          }}>Export CSV</button>
        </div>
      </div>
      ...
      {period === 'custom' && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
          <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ padding: '6px 10px', border: `1px solid ${t.border}`, borderRadius: 8, fontSize: 12 }} />
          <span style={{ color: t.textSecondary }}>to</span>
          <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ padding: '6px 10px', border: `1px solid ${t.border}`, borderRadius: 8, fontSize: 12 }} />
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={c.label} style={{ background: t.bgLight, border: `1px solid ${t.border}`, borderRadius: 12, padding: '16px 20px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {mechanics.length === 0 ? (
              ...
              <tr key={m.id} style={{ borderBottom: `1px solid ${t.bgHover}`, cursor: 'pointer' }}
                onClick={() => setSelectedMech(selectedMech?.id === m.id ? null : m)}
                onMouseEnter={e => (e.currentTarget.style.background = t.bgHover)}
                onMouseLeave={e => (e.currentTarget.style.background = '')}>
                <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 600, color: t.text }}>{m.full_name}</td>
                ...
                <td style={{ padding: '12px 14px', fontFamily: MONO, fontSize: 12, color: t.text }}>{m.hours_logged}h</td>
      ...
      {selectedMech?.jobs && (
        <div style={{ background: t.bgLight, border: `1px solid ${t.border}`, borderRadius: 12, padding: 20, marginTop: 16 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: t.text, marginBottom: 12 }}>{selectedMech.full_name} — Job Detail</div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${t.border}` }}>
            ...
            <tbody>
              {selectedMech.jobs.map((j: any) => (
                <tr key={j.id} style={{ borderBottom: `1px solid ${t.bgHover}` }}>
                  ...
                  <td style={{ padding: '10px 12px', fontSize: 12, color: t.text, maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{j.complaint || '--'}</td>
                  ...
                  <td style={{ padding: '10px 12px', fontFamily: MONO, fontSize: 12, color: t.text }}>{j.grand_total ? fmt(j.grand_total) : '--'}</td>
                </tr>
              ))}
            </tbody>
          </table>
